        ...
        HttpClientResponse.fromWeb(
          ...
          new Response(
            JSON.stringify({
              name: "r/button",
              type: "registry:ui",
              dependencies: ["react", "@ark-ui/react"],
              files: [
                {
                  path: "ui/button.tsx",
...
const setupReactApp = (dir: string) =>
  Effect.gen(function*() {
...
describe("React E2E Flow", () => {
  it.live("scaffolds react app, inits, and adds button", () =>
    withTempDir((dir) =>
      Effect.gen(function*() {
        ...
        // 1. Setup Mock App
        yield* setupReactApp(dir)

        // 2. Run Init
        // We use --install-deps=false to avoid running installs in test
        yield* cli([
          "node",
          "test",
          "init",
